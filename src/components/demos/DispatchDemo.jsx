...
import { IconGrid, IconTruck, IconUsers, IconChart, IconBell } from './icons.jsx'
...
export default function DispatchDemo() {
  ...
  const enrouteCount = drivers.filter((d) => d.status === 'enroute').length
  const deliveredCount = orders.filter((o) => o.status === 'delivered').length

  return (
    <div className="demo-theme--dispatch dt-shell dt-shell--app">
      <aside className="dt-sidebar">
        <div className="dt-sidebar__brand"><IconTruck /> Fleetline</div>
        <div className="dt-sidebar__item dt-sidebar__item--active"><IconGrid /> Dashboard</div>
        <div className="dt-sidebar__item"><IconTruck /> Drivers</div>
        <div className="dt-sidebar__item"><IconUsers /> Orders</div>
        <div className="dt-sidebar__item"><IconChart /> Reports</div>
      </aside>

      <div className="dt-main">
        <div className="dt-topbar">
          <div className="dt-topbar__title">Live dispatch board</div>
          <div className="dt-topbar__right">
            <IconBell />
            <img className="dt-avatar" src="https://i.pravatar.cc/64?u=fleetline-dispatcher" alt="Dispatcher" width="32" height="32" />
          </div>
        </div>

        <div className="dt-kpi-row">
          <div className="dt-card">
            <div className="dt-stat-value">{idleDrivers.length}</div>
            <div className="dt-stat-label">Idle drivers</div>
          </div>
          <div className="dt-card">
            <div className="dt-stat-value" style={{ color: 'var(--dt-accent)' }}>{enrouteCount}</div>
            <div className="dt-stat-label">En route now</div>
          </div>
          <div className="dt-card">
            <div className="dt-stat-value" style={{ color: '#4ade80' }}>{deliveredCount}/{orders.length}</div>
            <div className="dt-stat-label">Delivered today</div>
          </div>
        </div>

        <div className="dt-eyebrow">Drivers</div>
